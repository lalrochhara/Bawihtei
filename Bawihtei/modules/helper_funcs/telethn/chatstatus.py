from Bawihtei.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Bawihtei import DRAGONS
...
async def Bawihtei_is_admin(chat_id: int):
    status = False
    Bawihtei = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
        if Bawihtei.id == user.id:
            status = True
            break
    return status
